...
void test_decodificar_archivo() {
  ...
  BTree raiz = btree_unir(letra4, l, r);
  int p = 0;
  char* decodificado1 = decodificar_archivo(raiz, "00101001101", &p);
  int q = 0;
  char* decodificado2 = decodificar_archivo(raiz, "110001001", &q);
  assert(strcmp(decodificado1, "aceacce") == 0);
  assert(strcmp(decodificado2, "ccaeac") == 0);
  free(decodificado1);
  free(decodificado2);
  btree_destruir(raiz);
}
